...
def run_tv_pipeline():
    ...
    if not session_flag.exists():
        ...
        send_email(
            ...
            '<h3 style="color:#fff">To fix (Safari):</h3>'
            '<ol style="line-height:1.8">'
            '<li>Log into <a href="https://www.rtings.com/login" style="color:#60a5fa">rtings.com</a> — confirm top-right shows your username</li>'
            '<li>One-time Safari setup (skip if already done): <b>Safari → Settings → Advanced</b> → check <b>"Show features for web developers"</b></li>'
            '<li>Right-click the page → <b>Inspect Element</b></li>'
            '<li>In DevTools top bar click <b>Storage</b> → left sidebar expand <b>Cookies</b> → click <code style="background:#333;padding:2px 6px;border-radius:3px">https://www.rtings.com</code></li>'
            '<li>Find the row <code style="background:#333;padding:2px 6px;border-radius:3px">_rtings_session</code> → double-click the <b>Value</b> cell → copy</li>'
            ...
            '<li><i>(Optional, for local runs)</i> Also update <code style="background:#333;padding:2px 6px;border-radius:3px">RTINGS_SESSION=</code> in your local <code>.env</code></li>'
            '</ol>'
